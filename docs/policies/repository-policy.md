...
- Keep it limited to lifecycle wiring, settings/event delegation, and thin
  compatibility facades. Put workflows, persistence, UI, transport, and
  restart scheduling in focused thematic classes that do not implement Rising
  World's `Listener`. JDA listeners are unaffected.
